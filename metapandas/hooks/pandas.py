...
from metapandas.util import verr, vprint
...
# exported pandas functions (pre-wrapped)
read_csv = getattr(pd, "read_csv", None)
read_excel = getattr(pd, "read_excel", None)
read_feather = getattr(pd, "read_feather", None)
read_hdf = getattr(pd, "read_hdf", None)
read_json = getattr(pd, "read_json", None)
read_parquet = getattr(pd, "read_parquet", None)
read_pickle = getattr(pd, "read_pickle", None)
read_sql = getattr(pd, "read_sql", None)
read_sql_table = getattr(pd, "read_sql_table", None)
read_sql_query = getattr(pd, "read_sql_query", None)
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if meta_kwargs.get("argname_is_path", None) is False:
                    metadata.update({argname: kwargs.get(argname, args[0])})
                else:
                    ...
                    with open(metapath) as metafile:
                        metadata.update(
                            {"data_filepath": datapath, "metadata_filepath": metapath}
                        )
                        metadata.update(json.loads(metafile.read()))
            except IOError as err:
                vprint(
                    "Could not load metadata from {} due to {!r}".format(metapath, err),
                    file=sys.stderr,
                )
            except Exception as err:
                vprint(
                    "Error setting up metadata due to {!r}".format(err), file=sys.stderr
                )
            finally:
...
def pandas_save_with_metadata(
    function=None, argname="path", metadata=MetaData(), **meta_kwargs
):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            additional_data.update(
                {
                    "storage": {
                        "method": func,
                        **{
                            attr: getattr(arginfo, attr)
                            for attr in dir(arginfo)
            ...
            try:
                ...
                metadata.save_as_json(
                    filepath=metapath, data=data, additional_data=additional_data
                )
            except IndexError:
                pass  # unable to establish filename, so skip
            except Exception as err:
                verr("Could not save metadata to {} due to {!r}".format(metapath, err))
                raise
            return result
...
class PandasMetaDataHooks(HooksManager):
    ...
    def install_metadata_hooks(cls):
        ...
        applied_pd_hooks = cls.apply_hooks(
        ...
        if applied_pd_hooks or applied_df_hooks:
            vprint("Installed {} hooks".format(cls.__name__), file=sys.stderr)
        else:
            verr("*** {} hooks already installed ***".format(cls.__name__))
    ...
    def uninstall_metadata_hooks(cls):
        ...
        removed_df_hooks = cls.remove_hooks(pd.DataFrame, cls.PANDAS_DATAFRAME_SAVE_HOOKS)  # type: ignore
        if removed_pd_hooks or removed_df_hooks:
            verr("Uninstalled {} hooks".format(cls.__name__))
        else:
            verr("*** No {} hooks installed ***".format(cls.__name__))


# Now decorate MetaDataFrame to use metadata save decorators
with open(os.devnull, "w") as devnull:
    with redirect_stdout(devnull), redirect_stderr(devnull):
        PandasMetaDataHooks.apply_hooks(
            MetaDataFrame,
            pandas_save_with_metadata,
...
for method, meta_kwargs in PandasMetaDataHooks.PANDAS_READ_HOOKS.items():
    ...
    if func_method is None:
        verr('Warning: No such attribute to decorate: "pd.{}"'.format(method))
